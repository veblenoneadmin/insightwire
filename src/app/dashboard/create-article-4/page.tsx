...
export default function CreateArticle4Page() {
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', background: VS.bg0, fontFamily: "'DM Sans', sans-serif" }}>
      ...
      {/* ════════════════════════════════════════════════════
          STAGING PANEL — Full content, article & quotes
          ════════════════════════════════════════════════════ */}
      {(stagedContent || stagedQuotes.length > 0) && (
        <div style={{ width: '420px', minWidth: '420px', overflowY: 'auto', borderRight: `1px solid ${VS.border}`, background: VS.bg1, flexShrink: 0 }}>
          <div style={{ padding: '16px' }}>
            <div style={{ fontFamily: 'monospace', fontSize: '10px', color: '#ce93d8', textTransform: 'uppercase', letterSpacing: '1.5px', marginBottom: '14px', fontWeight: 600 }}>Content & Quotes</div>

            {/* Staged Announcement Content */}
            {stagedContent && (
              <div style={{ marginBottom: '14px' }}>
                ...
                <div style={{ fontSize: '10px', color: VS.text2, marginBottom: '6px', fontFamily: 'monospace' }}>
                  {contentEditMode ? 'Edit mode — changes sync to the article generation.' : 'Saved quotes highlighted. Click Edit to modify.'}
                </div>
                {contentEditMode ? (
                  <textarea
                    ...
                    style={{ ...inp, minHeight: '220px', fontSize: '11px', lineHeight: 1.6, resize: 'vertical' }}
                  />
                ) : (
                  <div style={{ padding: '10px 12px', background: VS.bg2, border: `1px solid ${VS.border}`, borderRadius: '4px', maxHeight: '320px', overflowY: 'auto', fontSize: '11px', color: VS.text1, lineHeight: 1.6, whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
                    {(() => {
            ...
            >
              {articleLoading ? <><Loader2 size={14} style={{ animation: 'spin 1s linear infinite' }} /> Generating…</> : <><CheckCircle2 size={14} /> Generate Article</>}
            </button>
            {!brief && (
              <div style={{ marginTop: '8px', fontSize: '10px', color: VS.error, fontFamily: 'monospace', textAlign: 'center' }}>
          ...
          {!hasArticle && !articleLoading && (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', padding: '60px 40px', textAlign: 'center', color: VS.text2, gap: '14px' }}>
              ...
              <p style={{ fontSize: '13px', maxWidth: '320px', lineHeight: 1.6, margin: 0 }}>
                Generate the article from the Content &amp; Quotes column.
              </p>
            </div>
          )}
